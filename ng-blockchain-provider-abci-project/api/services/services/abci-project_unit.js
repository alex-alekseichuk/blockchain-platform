...
// 3. In Car 2 (from Bob)
    ...
    expect(history[0].tmId).to.be.equal(transactions[0].tmId);
    expect(history[1].tmId).to.be.equal(transactions[5].tmId);
    expect(history[2].tmId).to.be.equal(transactions[6].tmId);
  });
  ...
  it('Get History of car asset for alice', async () => {
    ...
    expect(history[0].tmId).to.be.equal(transactions[9].tmId);
  });
});
...
describe('test getOutputs()', () => {
  ...
  it('get outputs', async () => {
    ...
    expect(output).to.have.property('assetType').equal(assetType);
  });
...
describe('Test getInputs()', () => {
  ...
  it('get inputs', async () => {
    ...
    expect(inputs[0].txData).to.have.property('operation');
    expect(inputs[0].txData).to.have.property('outputs');
    expect(inputs[0].txData).to.have.property('inputs');
    expect(inputs[0].txData).to.have.property('version');
    expect(inputs[0].txData).to.have.property('metadata');
    expect(inputs[0].txData).to.have.property('asset');
    expect(inputs[0]).to.have.property('assetType');
    expect(inputs[0]).to.have.property('assetFormat');
  });
...
describe('Test getAssetsByOwner()', () => {
  ...
  it('get all assets true', async () => {
    ...
    expect(assets[0]).to.have.property('serialNumber').eql('56288320528682648929801');
    expect(assets[0]).to.have.property('Type').eql('car');
    expect(assets[0]).to.have.property('modelName').eql('BMW x');
    expect(assets[0]).to.have.property('registrationYear').eql('2019');
    expect(assets[0]).to.have.property('colour').eql('black');
    expect(assets[0]).to.have.property('numberOfDoors').eql('3');
    expect(assets[0]).to.have.property('fueltype').eql('Diesel');
    expect(assets[0]).to.have.property('time').eql('1561971470219');
  });
  ...
  it('get all assets true', async () => {
    ...
    expect(assets[0]).to.have.property('numberOfDoors').eql('3');
    expect(assets[0]).to.have.property('fueltype').eql('Diesel');
    expect(assets[0]).to.have.property('time').eql('1562053828020');
  });
