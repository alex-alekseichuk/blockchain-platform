...
let component = {
  ...
  onSubmit: function() {
    // console.log("**********3234234* additionalMsg ***23423424***** ");
    // console.log(this.additionalMsg);
    ...
    new Promise((resolve, reject) => {
      ...
      resolve();
            ...
            .then(() => {
              let spaEngine = document.querySelector('#documentsSharing');
              let listener = () => {
                if (document.querySelector(component.is)) {
                  let sharedTo = _.chain(self.recipients)
                            .filter(i => i.pubKey)
                            .map(i => i.pubKey)
                            .value();

                  project.toast.show(`${translate('Shared to emails: ')}${sharedTo.join(', ')}`);

                  spaEngine.$.staticHtmlPage
                            .removeEventListener('dom-change', listener);
                }
              };

              spaEngine.$.staticHtmlPage
                    .addEventListener('dom-change', listener);
            })
            .catch(err => {
              self.stopCopyAnim();
